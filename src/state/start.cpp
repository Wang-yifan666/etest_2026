...
namespace etest::state
{
	...
	State runStart(AppContext& ctx, const RuntimeConfig& runtime,
	               const SearchConfig& search_cfg, bool show_preview)
	{
		...
		{
			const int timeout_ms = 1500; // handshake_timeout_ms

			// 步骤 4a：等待可选的 BOOT,OK（下位机启动后主动发送）
			{
				UartMessage msg;
			...
			// 步骤 4c：发送 PROTO? 并等待 PROTO,4
			if(ctx.lower_machine_online
			   || true) // 即使 PING 超时也尝试 PROTO
			{
