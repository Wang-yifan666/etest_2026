...
void logEffectiveConfig(const etest::ConfigLoadResult& result)
{
	...
	ETEST_LOG_INFO("CONFIG",
	               "mode.enabled="
	                   + std::string(config.mode.enabled ? "true"
	                                                     : "false"));
	...
	ETEST_LOG_INFO("CONFIG",
	               "runtime.headless="
	                   + std::string(config.runtime.headless
	                                     ? "true"
	                                     : "false")
	                   ...
	                   + std::string(config.runtime.enable_self_check
	                                     ? "true"
	                                     : "false")
	...
	ETEST_LOG_INFO("CONFIG",
	               "logger.directory=" + config.logger.directory
	                   ...
	                   + (config.logger.flush_each_write ? "true"
	                                                     : "false")
	...
	ETEST_LOG_INFO("CONFIG",
	               "vision parameters: "
	               "h1=[" + std::to_string(config.vision.red_h1_min)
	                   + "," + std::to_string(config.vision.red_h1_max)
	                   + "] h2=["
	                   + std::to_string(config.vision.red_h2_min)
	                   + "," + std::to_string(config.vision.red_h2_max)
	                   + "] S_min="
	                   + std::to_string(
	                       config.vision.saturation_min)
	                   + " V_min="
	                   + std::to_string(config.vision.value_min)
	                   + " min_area="
	                   + std::to_string(config.vision.min_area)
	                   + " kernel="
	                   + std::to_string(
...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		const bool allow_keyboard_exit = cfg.runtime.allow_keyboard_exit;
		...
		while(ctx.running)
		{
			...
			catch(const cv::Exception& error)
			{
				...
				ctx.last_fault = {etest::FaultSource::VISION,
