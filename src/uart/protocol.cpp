...
namespace etest::uart::protocol
{
	...
	bool hexToCrc16(const std::string& hex, std::uint16_t& out)
	{
		...
		for(char ch: hex)
		{
			...
			{
				value |= static_cast<std::uint16_t>(ch - 'A' + 10);
			}
			else if(ch >= 'a' && ch <= 'f')
			{
				value |= static_cast<std::uint16_t>(ch - 'a' + 10);
	...
	DecodedMessage decodeFrame(const std::string& frame)
	{
		...
		{
			result.error =
			    "frame too long: " + std::to_string(frame.size())
		...
		for(int i = 0; i < 4; ++i)
		{
			...
			{
				result.error = "missing field " + std::to_string(i + 1)
