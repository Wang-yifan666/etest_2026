...
namespace
{
	                      ...
	                      etest::ConfigLoadResult& result)
	{
		   ...
		   || (!allow_empty && value.empty()))
		{
			addMessage(
			    ...
			    key + " is invalid at line " + std::to_string(raw->line)
	...
	bool getBool(const RawConfig& raw_config, const std::string& key,
	             bool default_value, etest::ConfigLoadResult& result)
	{
		const RawValue* raw = findValue(raw_config, key);
	           ...
	           etest::ConfigLoadResult& result)
	{
		...
		{
			addMessage(result, etest::ConfigMessageLevel::WARNING,
			           key + " is outside [" + std::to_string(minimum)
			               + ", " + std::to_string(maximum)
			               + "] at line " + std::to_string(raw->line)
			               + "; using default "
			               + std::to_string(default_value));
	                            ...
	                            etest::ConfigLoadResult& result)
	{
		...
		addMessage(result, etest::ConfigMessageLevel::ERROR,
		           key + " has unknown level \"" + value + "\" at line "
		               + std::to_string(raw->line)
		               + "; using default INFO");
	                    ...
	                    etest::ConfigLoadResult& result)
	{
		...
		while(std::getline(input, line))
		{
			...
			raw_config[full_key] = RawValue{value, line_number};
		}
	                     ...
	                     etest::ConfigLoadResult& result)
	{
		...
		{
			if(known_keys.find(item.first) == known_keys.end())
			{
				addMessage(result, etest::ConfigMessageLevel::WARNING,
				           "unknown key " + item.first + " at line "
	                      ...
	                      etest::ConfigLoadResult& result)
	{
		...
		config.logger.file = getBool(raw_config, "logger.file",
		                             config.logger.file, result);
		...
		if(!config.logger.file && !config.logger.terminal)
		{
			...
			addMessage(
			    result, etest::ConfigMessageLevel::WARNING,
		...
		config.camera.fps = getInt(raw_config, "camera.fps",
		                           config.camera.fps, 1, 240, result);
...
namespace etest
{
	    ...
	    const std::string& path) noexcept
	{
		...
		try
		{
			...
			const RawConfig raw_config = parseFile(input, result);
	    ...
	    const std::vector<std::string>& paths) noexcept
	{
		...
		for(const auto& path: paths)
		{
			try
			{
				...
				const RawConfig file_config = parseFile(input, result);
		...
		extractAppConfig(result.config, merged, result);
