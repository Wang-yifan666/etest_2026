...
void fallback(
    etest::LogLevel level,
    const std::string& source,
    const std::string& message) noexcept
{
    std::fprintf(
        stderr,
        "[LOGGER FALLBACK] %s - %s - %s\n",
        levelName(level),
        source.c_str(),
        message.c_str());
...
bool Logger::init(
    const LoggerConfig& config) noexcept
{
    try
    {
        ...
        write(
            ...
            "LOGGER",
            file_.is_open()
                ? "initialized, file=" + file_path_
                : "initialized, file output disabled",
            true);

        return config_.terminal ||
               file_.is_open();
    }
    ...
    std::string& error) noexcept
{
    ...
    {
        error =
            std::string("open log file exception: ") +
            exception.what();
    ...
    {
        error =
            "unknown open log file exception";
