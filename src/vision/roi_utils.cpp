...
namespace etest::vision::roi_utils
{
	...
	{
		const int y =
		    std::clamp(pipe_center_y - roi_height / 2, 0,
	...
	{
		const int x =
		    std::clamp(pipe_center_x - roi_width / 2, 0,
	                                 ...
	                                 const BallNcnnConfig& config)
	{
		...
		if(config.roi_location_mode == "topleft")
		{
			// 直接使用左上角坐标
			roi.rect =
			    cv::Rect(config.full_roi_x, config.full_roi_y,
			             config.full_src_width, config.full_src_height);
		}
		else
		{
			// 旧逻辑：通过中心点反算
			roi.rect = makeFullRoi(frame_size, config.full_src_width,
			                       config.full_src_height,
			                       config.pipe_center_y);
		}

		roi.model_input_size =
		    cv::Size(config.full_input_width, config.full_input_height);
	                                   ...
	                                   const BallNcnnConfig& config)
	{
		...
		{
			roi.rect = cv::Rect(
			    config.center_roi_x, config.center_roi_y,
			    config.center_src_width, config.center_src_height);
		}
		else
		{
			roi.rect = makeCenterRoi(
			    frame_size, config.center_src_width,
			    config.center_src_height, config.pipe_center_x,
			    config.pipe_center_y);
		}
	                 ...
	                 const PipeAxisCalibration& cal)
	{
		...
		if(cal.image_right_sign != -1 && cal.image_right_sign != 1)
		{
			return 0.0;
		}
		...
		for(std::size_t i = 1; i < pts.size(); ++i)
		{
			if(pixel_global <= pts[i].pixel)
			{
				...
				const double ratio =
				    (pixel_global - p0.pixel) / (p1.pixel - p0.pixel);
				return (p0.position_mm
				        + ratio * (p1.position_mm - p0.position_mm))
	    ...
	    const PipeAxisCalibration& cal) noexcept
	{
		...
		result.valid = true;
		result.position_mm = pixelToMm(pixel_global, cal);
