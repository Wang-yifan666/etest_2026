...
bool test_start_stop()
{
	...
	if(capture.start())
	{
		std::cerr << "FAIL: start should fail when camera not opened\n";
		return false;
	}
...
bool test_try_get_latest_empty()
{
	...
	if(got)
	{
		std::cerr << "FAIL: tryGetLatest should return false when no frames\n";
		return false;
	}
	...
	{
		std::cerr << "FAIL: output should not be modified on failure\n";
		return false;
	}
...
bool test_with_video_file()
{
	...
	if(st != CaptureWorkerState::RUNNING && st != CaptureWorkerState::FILE_EOF)
	{
		std::cerr << "FAIL: unexpected state " << static_cast<int>(st) << "\n";
		capture.stop();
		return false;
	}
	...
	if(!got)
	{
		std::cerr << "FAIL: tryGetLatest should return true after capture\n";
		capture.stop();
		return false;
	}

	std::cout << "  got frame sequence=" << output.sequence
	          << " size=" << output.frame.cols << "x" << output.frame.rows
	          << "\n";
	...
	if(got)
	{
		...
		std::cout << "  second tryGetLatest returned " << (got ? "true" : "false") << "\n";
	}
...
bool test_single_slot_overwrite()
{
	...
	if(f == nullptr)
	{
		std::cout << "SKIP: test_single_slot_overwrite (no test video)\n";
		return true;
	}
	std::fclose(f);
	...
	if(diff > 3)
	{
		...
		{
			std::cerr << "FAIL: consumer sequence too far behind (diff=" << diff << ")\n";
			capture.stop();
			return false;
		}
	}
	...
	{
		std::cerr << "FAIL: consumer got frame 1, should have gotten a later frame\n";
		capture.stop();
		return false;
	}
