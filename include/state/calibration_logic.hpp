...
namespace etest
{
	namespace state
	{
		...
		/// 球是否位于标定线容差带内。
		/// 只比较 X 坐标，Y 不受限制。
		bool isBallOnCalibrationLine(float global_x,
		...
		CalibrationGateResult updateCalibrationGate(
		    CalibrationGateState& state, float global_x,
		    float calibration_line_x, int required_frames,
		    float max_jitter_px, bool on_line) noexcept;
